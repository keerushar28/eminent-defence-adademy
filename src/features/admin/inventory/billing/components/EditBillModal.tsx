...
import { updateBill } from "../../actions/billing-actions";
...
export default function EditBillModal({ isOpen, onClose, onSuccess, bill, isAdmin = false }: EditBillModalProps) {
    ...
    useEffect(() => {
        if (isOpen && bill) {
            form.setValue("categoryId", bill.categoryId);
            form.setValue("billingTitle", bill.billingTitle);
            form.setValue("amount", bill.amount.toString());
            form.setValue("units", bill.units ? bill.units.toString() : "");
            form.setValue("description", bill.description || "");
            
            setBillDateAD(new Date(bill.billDate));
            setPeriodStartAD(new Date(bill.periodStartDate));
            setPeriodEndAD(new Date(bill.periodEndDate));
            
            form.setValue("billDate", convertDateToYYYYMMDD(new Date(bill.billDate)));
            form.setValue("periodStartDate", convertDateToYYYYMMDD(new Date(bill.periodStartDate)));
            form.setValue("periodEndDate", convertDateToYYYYMMDD(new Date(bill.periodEndDate)));
        }
    }, [isOpen, bill, form]);
        ...
        try {
            ...
            if (result.success) {
                ...
                const updatedBill: Bill = {
                    ...
                    periodStartDate: new Date(values.periodStartDate),
                    periodEndDate: new Date(values.periodEndDate),
        ...
        <Sheet open={isOpen} onOpenChange={handleClose}>
            <SheetContent className="w-full sm:max-w-xl gap-0 pb-2 p-6 h-full font-medium overflow-y-auto">
                <SheetHeader className="mb-2 p-0 border-b pb-4 gap-0.5">
                    <SheetTitle className="text-xl">Edit Bill</SheetTitle>
                    <SheetDescription className="text-sm font-normal">
                        Update utility bill or expense details.
                    </SheetDescription>
                </SheetHeader>
                    ...
                    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4 pt-4">
                                        ...
                                        <SelectContent>
                                            {categories.length === 0 ? (
                                                <div className="p-2 text-sm text-muted-foreground">
                                                    No billing categories available.
                                                </div>
                                            ) : (
                                                categories.map((cat) => (
                        ...
                        <div className="flex justify-end gap-2 pt-4 border-t">
                            ...
                            <Button type="submit" disabled={isSubmitting}>
                                {isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Update Bill"}
                            </Button>
                        </div>
                    </form>
                </Form>
