...
import { IInventoryCategory, IVendor } from "../types/inventory-types";
...
export default function AddItem({ isOpen, onClose, onSuccess, categories }: AddItemProps) {
    ...
    try {
      ...
      if (result.success) {
        toast.success("Item Created Successfully! 🎉", {
          description: `Item "${values.name}" has been added.`,
          duration: 5000,
        });
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="w-full sm:max-w-xl gap-0 pb-2 p-6 h-full font-medium overflow-y-auto">
        <SheetHeader className="mb-2 p-0 border-b pb-4 gap-0.5">
          <SheetTitle className="text-xl">Add Inventory Item</SheetTitle>
          <SheetDescription className="text-sm font-normal">
            Create a new inventory item. All fields marked with * are required.
          </SheetDescription>
        </SheetHeader>
